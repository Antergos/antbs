...
build_32bit_pkg() {
	...
	_2log 'CREATING 32-BIT BUILD ENVIRONMENT';
	setup_32bit_env
	_2log 'UPDATING 32BIT SOURCE CHECKSUMS'
...
try_build() {
	...
	if [[ "$1" = "i686" ]]; then
		...
		cd /pkg
		_2log 'UPDATING SOURCE CHECKSUMS';
		check_pkg_sums &&
...
build_package_or_prepare_to_update_repo() {
	...
	if pkgbuild_produces_i686_package; then
		...
		{ try_build 2>&1 && touch /result && exit 0; } || exit 1
	fi
